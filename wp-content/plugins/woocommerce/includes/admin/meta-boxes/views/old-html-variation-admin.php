...
<div class="woocommerce_variation wc-metabox closed">
	<?php if($_POST['action'] == 'woocommerce_add_variation'):?>
		<table>
			<tr>
				<td width="5%;"><strong>#<?php echo esc_html( $variation_id ); ?></strong></td>
				<?php foreach ( $parent_data['attributes'] as $attribute ) {
				...
				<td width="10%">
					<input type="text" size="5" name="variable_regular_price[<?php echo $loop; ?>]" value="<?php if ( isset( $_regular_price ) ) echo esc_attr( $_regular_price ); ?>" class="wc_input_price"/>
			...
			<tr>
				...
				<?php foreach ( $parent_data['attributes'] as $attribute ) {
					...
					// Name will be something like attribute_pa_color
                                        echo '<td width="'.(85/count($parent_data['attributes'])).'%">';
                                        echo '<input type="hidden" name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']" value="'.esc_attr($variation_selected_value).'">';
					if ( $attribute['is_taxonomy'] ){
						$post_terms = wp_get_post_terms( $parent_data['id'], $attribute['name'] );

						foreach ( $post_terms as $term ) {
							if($variation_selected_value == $term->slug){
								echo apply_filters( 'woocommerce_variation_option_name', esc_html( $term->name ) );
							}
						}
	...
	<table cellpadding="0" cellspacing="0" class="woocommerce_variable_attributes wc-metabox-content" style="display:none;">
		<tbody>
			<tr>
			...
			<tr>
				...
				<td class="options">
					<label><input type="checkbox" class="checkbox" name="variable_enabled[<?php echo $loop; ?>]" checked="checked" /> <?php _e( 'Enabled', 'woocommerce' ); ?></label>
					...
					<label><input type="checkbox" class="checkbox variable_is_virtual" name="variable_is_virtual[<?php echo $loop; ?>]" checked="checked" /> <?php _e( 'Virtual', 'woocommerce' ); ?> <a class="tips" data-tip="<?php _e( 'Enable this option if a product is not shipped or there is no shipping cost', 'woocommerce' ); ?>" href="#">[?]</a></label>
