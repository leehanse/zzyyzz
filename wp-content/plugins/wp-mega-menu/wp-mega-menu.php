			...
			if ($show == 'both' && is_taxonomy_hierarchical($args['object'])) {
				...
				if (!empty($categories)) {
					...
					foreach ($categories as $cat) {
						...
						$return .= '<a href="'.get_category_link($cat->term_id).'" class="'.$class.'-subcategory'.$current.'" data-new_object_id="'.$cat->term_id.'"><i class="fa fa-angle-right"></i> '.$cat->name.'</a> ';
					}
					$return .= '</div>';
			...
			if (is_taxonomy_hierarchical($args['object'])) {
				...
				if (!empty($categories)) {
					...
					foreach ($categories as $i => $cat) {
						...
						$return .= '<h4 class="'.$class.'-subcategory-heading"><a href="'.get_category_link($cat->term_id).'">'.$cat->name.'</a></h4>';
						...
						if (!empty($subcategories)) {
							foreach ($subcategories as $subcat) {
								$return .= '<a href="'.get_category_link($subcat->term_id).'" class="'.$class.'-subcategory-child"><i class="fa fa-angle-right"></i> '.$subcat->name.'</a>';
							}
						}
						$return .= '</div>';
					}
...
function wpmm_get_the_posts( $query_args ) {
	...
	if ($query_args['wpmm_pagination'] && ($query_args['paged'] > 1 || $last_page > $query_args['paged'])) {
		...
		if ($query_args['paged'] > 1)
			$return .= '<a href="#" class="'.$class.'-pagination-previous" data-page="'.($query_args['paged']-1).'"><i class="fa fa-angle-left"></i></a>';
		if ($last_page > $query_args['paged'])
			$return .= '<a href="#" class="'.$class.'-pagination-next" data-page="'.($query_args['paged']+1).'"><i class="fa fa-angle-right"></i></a>';
		$return .= '</div>';
	}
