...
function my_custom_post_paper_size(){
	$labels = array(
	...
	$args = array(
		'labels'        => $labels,
		'description'   => 'Holds our paper_sizes and paper_size specific data',
		'public'        => true,
		'menu_position' => 12,
...
add_action( 'init', 'my_custom_post_paper_size' );
...
function paper_size_updated_messages( $messages ) {
	...
	$messages['paper_size'] = array(
		0 => '', 
		1 => sprintf( __('Paper Size updated. <a href="%s">View paper_size</a>'), esc_url( get_permalink($post_ID) ) ),
		...
		6 => sprintf( __('Paper Size published. <a href="%s">View paper_size</a>'), esc_url( get_permalink($post_ID) ) ),
		7 => __('Paper Size saved.'),
		8 => sprintf( __('Paper Size submitted. <a target="_blank" href="%s">Preview paper_size</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
		9 => sprintf( __('Paper Size scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview paper_size</a>'), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
		10 => sprintf( __('Paper Size draft updated. <a target="_blank" href="%s">Preview paper_size</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
	);
	return $messages;
}
...
function paper_size_contextual_help( $contextual_help, $screen_id, $screen ) { 
	if ( 'paper_size' == $screen->id ) {
		...
		$contextual_help = '<h2>Editing paper_sizes</h2>
		<p>This page allows you to view/modify paper size details. Please make sure to fill out the available boxes with the appropriate details (paper_size image) and <strong>not</strong> add these details to the paper_size description.</p>';
