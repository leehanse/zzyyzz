<?php
/**
 * Template Name: Renewal Template
 */
?>
<?php 
  ...
  define('ERROR_RENEWAL','Please double check the information you entered and try again. If problems continue, please contact subscriptions@imbibemagazine.com or call 1-877-246-2423 for assistance.');
  ...
  if(isset($_POST['action']) && $_POST['action'] == 'find'){    
    ...
    try{
        global $wpdb;
        if($SubscriptionNum){
            $sql  = $wpdb->prepare("SELECT * FROM customer_renewals WHERE SubscriptionNum = %s", $SubscriptionNum);        
            $row  = $wpdb->get_row($sql);
            
            $sql   = $wpdb->prepare("SELECT * FROM customer_digitals WHERE SubscriptionNum = %s", $SubscriptionNum);
            $row1  = $wpdb->get_row($sql);            
            if(!$row && !$row1)throw new Exception(ERROR_RENEWAL);
            ...
            if($ShiptoName && $ShiptoCity && $ShiptoZip){
                ...
                $row  = $wpdb->get_row($sql);
                if(!$row)throw new Exception(ERROR_RENEWAL);                
            }else throw new Exception(ERROR_RENEWAL);
        }
        
        $subscribe_page = get_page_by_title('Subscribe');
        if($subscribe_page){
            $subscribe_url = get_permalink(get_page_by_title('Subscribe')->ID);
            $subscribe_url = add_query_arg(array('action'=>'fill','promo'=>'renew','sn'=>$row->SubscriptionNum,'SourceCode'=>$source),$subscribe_url);
...
<div class="content clearfix">
    <div class="panelCenter">
        ...
        <section class="Drinkup">
                ...
                <div class="right">
                    <h1 style="padding-top:0px;margin-top: 0px;">Renewal Instructions</h1>
                    <section class="formShop">                        
                        <p>Access your subscription information and special renewal savings using the secure form below. You will be able to update your address on the next page.</p>
                        <p>Please enter either your full subscription number OR three other pieces of information to begin your renewal. 
                        Click <?php if($popup_url == '#'):?>
                            <a href="#">here</a>
                        <?php else:?>
                    ...
                    ?>
                    <form method="post" id="renewal_form" action="<?php echo add_query_arg(array('promo'=>$promo,'SourceCode'=>$source),  get_permalink());?>">
        ...
        <script type="text/javascript">
            $(document).ready(function(){
               $("#renewal_form input[type='text']").keyup(function( event ) {
                    if ( event.which == 13 ) {    
                        submitFormProcess();
                    }
               });
            ...
            function submitFormProcess(){
                ...
                if(check_form) $("#renewal_form").submit();
                else alert('<?php echo ERROR_RENEWAL;?>');
            }
        </script>    
    </div>
</div>
