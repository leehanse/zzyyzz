...
<?php if($article_type == "image"):?>            
        <?php if(custom_post_thumbnail(get_the_ID(),null,null,false,false)):?>
            <div class="thumb" id="thumb-<?php echo get_the_ID();?>">
                <?php custom_post_thumbnail(null,null,null,true,false);?>        
            </div>
        <?php endif;?>
        <script type="text/javascript">
            function modifyThumbnail<?php echo get_the_ID();?>(){
                jQuery("#thumb-<?php echo get_the_ID();?>").each(function(){
                    width = jQuery(this).find("img").width();                    
                    if(width > 307){
                        jQuery(this).addClass("thumbCenter").removeClass("thumb");
                    }
                });
            }
            jQuery(document).ready(function(){
                interv<?php echo get_the_ID();?> = window.setInterval("modifyThumbnail<?php echo get_the_ID();?>();",1000);
            });
        </script>
        <?php the_content();?>
